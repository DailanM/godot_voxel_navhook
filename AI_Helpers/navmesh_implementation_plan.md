...
Controls where navmesh is generated by proximity. See [plan reference — Navigation Viewer](navmesh_integration_plan_reference.md#navigation-viewer).
...
- [ ] Create `terrain/navigation/voxel_nav_viewer.h`
  ...
  - Internal: `ViewerID _viewer_id` (or a separate nav viewer ID type)
- [ ] Create `terrain/navigation/voxel_nav_viewer.cpp`
  - `NOTIFICATION_ENTER_TREE`: register with `NavMeshManager` (if terrain has one) or a central nav viewer registry
  - `NOTIFICATION_EXIT_TREE`: unregister (with deferred unregistration for reparenting safety, matching `VoxelViewer` pattern)
  - `NOTIFICATION_TRANSFORM_CHANGED`: update position in registry
  - `_bind_methods()`: expose `nav_distance`, `enabled_in_editor`
- [ ] Add nav viewer registry to `NavMeshManager`:
  - `StdVector<NavViewerState>` with position and distance per viewer
  - `_is_within_nav_range(Vector3i chunk_pos)` checks against all registered nav viewers
  - Thread-safe read access (nav viewers update on main thread, readiness checks happen on worker threads)
...
- [ ] In `~VoxelTerrain()` destructor (matching the existing pattern where `_meshing_dependency->valid = false` is set in the destructor, not in `NOTIFICATION_EXIT_TREE`):
...
- [ ] Implement `_recompute_nav_config()` helper that derives rcConfig from properties and stores on the manager (see [plan reference — Property derivation](navmesh_integration_plan_reference.md#exposed-properties))
...
- [ ] In VoxelTerrain: when a mesh block is unloaded in `unload_mesh_block()` (`voxel_terrain.cpp:528`), call `_nav_mesh_manager->remove_chunk(bpos)` if manager exists

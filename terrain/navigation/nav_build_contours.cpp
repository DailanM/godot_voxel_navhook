...
// ============================================================================
// WalkSegment infrastructure
// ============================================================================
// Ref: RecastContour.cpp:103-184 (stock walkContour mechanics)
//
// Replaces the monolithic walkContour with a primitive walkSegment plus
// orchestrators (walkInteriorContour, walkBorderContour). Decomposing the
// walk lets the inland sub-walk for T-vertex side-trips (Phase 4+) reuse the
// same primitive: at a T-vertex, walkSegment stops, the orchestrator runs an
// inland walkSegment with FLAG_CLEAR_NEVER and stop_at_mandatory_*, simplifies
// the result, and injects the inland clip-point into the parent points[].
//
// Configuration knobs (WalkSegmentConfig):
// - flag_policy:
//     FLAG_CLEAR_ALWAYS  — interior walks (stock behavior)
//     FLAG_CLEAR_BORDER  — border walks: only clear flags on RC_BORDER_REG
//                          spans, and only during forward (non-reversed)
//                          phase. This policy ALSO enables internal reversal
//                          flipping at mandatory non-RC_BORDER_VERTEX
//                          emissions (chunk corners, hole-boundary
//                          transitions).
//     FLAG_CLEAR_NEVER   — inland sub-walks: preserve flags so the eventual
//                          interior contour walk on these spans isn't
//                          starved by our side-trip.
// - stop_at_mandatory_non_bv: when true, walkSegment returns on emission of
//     a mandatory non-RC_BORDER_VERTEX vertex (used by inland sub-walks to
//     stop at the inland clip endpoint).
// - stop_at_mandatory_bv: when true, walkSegment returns on emission of a
//     mandatory RC_BORDER_VERTEX (T-vertex). Used by the border orchestrator
//     in Phase 4+ to trigger the side-trip.
// - block_t_junction_steps: T-junction guard. During the reversed phase of a
//     border walk, don't step into non-border cells; rotate CW instead.
//
// Stop semantics: when a stop condition matches, walkSegment returns AFTER
// the emission, prev_r update, reversal flip (if border policy), and flag
// clearing (per policy), but BEFORE the dir rotation. state.dir is therefore
// the just-emitted boundary edge direction. The orchestrator is responsible
// for any post-stop bookkeeping (e.g., side-trip) and rotation:
//     state.dir = state.reversed ? (state.dir + 3) & 0x3 : (state.dir + 1) & 0x3;
// before re-entering walkSegment.

enum WalkStopReason {
	WALK_STOP_LOOP_CLOSED,
	WALK_STOP_AT_MANDATORY_NON_BV,
	WALK_STOP_AT_MANDATORY_BV,
	WALK_STOP_AT_DEAD_END,
};
...
struct WalkState {
	int x, y, i;
	unsigned char dir;
	int prev_r; // r value of most-recently-emitted vertex (-1 if no emission yet)
	bool reversed; // border-walk reversed phase flag
	int iter; // shared iteration counter (preserves the 40000 cap across stops)
};

struct WalkSegmentConfig {
	unsigned char area; // baseline area for RC_AREA_BORDER detection (seed span's area)
	WalkFlagPolicy flag_policy;
	bool stop_at_mandatory_non_bv;
	bool stop_at_mandatory_bv;
	bool block_t_junction_steps;
};
...
static WalkStopReason walkSegment(
		WalkState &state,
...
static void findAndInjectClipPoint(
	...
	walkSegment(inland_state, B_i, inland_dir, chf, flags, inland_raw, inland_cfg);
...
static void walkBorderContour(rcContext *ctx, int x, int y, int i,
		const rcCompactHeightfield &chf,
		unsigned char *flags, rcIntArray &points,
		float maxError) {
	unsigned char dir = 0;
